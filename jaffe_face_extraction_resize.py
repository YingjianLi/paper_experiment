#!/usr/bin/env python
#coding=utf-8
#"F:/face_data/jaffe123" 中的图像截出人脸保存到"F:/face_data/jaffe123_faces"中
...
def detect_object(input_path, output_path,count):
    ...
    if len(faceRects) > 0: #大于0则检测到人脸
        ...
        for faceRect in faceRects:
            ...
            file_name = os.path.join(output_path,str(count)+".tiff")
            #保存
            a=img.save(file_name,"tiff", quality=100)
...
def process(file_path):
    output_path = 'F:/face_data/'+'/'+'jaffe123_faces'
    try:
    ...
    for index in range(213):
        input_path = file_path+'/'+str(index+1)+'.tiff'
        print (input_path)
